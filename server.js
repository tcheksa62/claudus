...
io.on('connection', (socket) => {
  ...
  socket.on('reconnect-to-session', ({ sessionId, pseudo }) => {
    ...
    const isAdmin = socket.id === session.adminId || session.adminId === pseudo;
    if (isAdmin) {
    ...
    socket.emit('reconnected-to-session', {
      ...
      currentWord: session.gameStarted ? {
        length: session.words[session.currentWordIndex].length,
        firstLetter: session.words[session.currentWordIndex][0],
        wordNumber: session.currentWordIndex + 1,
        totalWords: session.words.length
      } : null
    });

    console.log(`${pseudo} reconnected to session ${sessionId}`);
  });
  ...
  socket.on('submit-guess', ({ sessionId, guess }) => {
    ...
    if (!words.includes(guessUpper)) {
      socket.emit('invalid-word', { message: 'Word not in dictionary' });
      return;
    }
